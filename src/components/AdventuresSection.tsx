...
export default function AdventuresSection() {
  ...
  const handlePrevGroup = () => {
    setActiveGroupIndex((prev) => (prev === 0 ? trekGroups.length - 1 : prev - 1));
    setActiveRouteIndex(0); // Reset route when changing start point
  };

  const handleNextGroup = () => {
    setActiveGroupIndex((prev) => (prev === trekGroups.length - 1 ? 0 : prev + 1));
    setActiveRouteIndex(0); // Reset route when changing start point
  };

  // Image fallback logic
  const mainImage = currentRoute?.images?.main || "/images/adventure-main.png";
  // Fallback to main image if small ones are missing, or a default placeholder
  const smallImage1 = currentRoute?.images?.gallery?.[0] || "/images/adventure-small-1.png";
  const smallImage2 = currentRoute?.images?.gallery?.[1] || "/images/adventure-small-2.png";
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex flex-col gap-4">
                  <motion.p variants={itemRevealVariants} className="font-montserrat font-medium text-lg md:text-xl text-[#f5f2e9] capitalize">
                    {currentGroup.startPoint} to
                  </motion.p>
                  <motion.p variants={itemRevealVariants} className="font-baron text-2xl md:text-3xl text-[#d4af37]">
                    {currentRoute.title}
                  </motion.p>
                </div>
                <motion.p variants={itemRevealVariants} className="font-montserrat font-medium text-base md:text-lg text-[#f5f2e9] leading-relaxed">
                ...
                {currentGroup.routes.map((route, index) => (
                  <button
                    ...
                    className={`flex flex-col gap-4 items-start p-0 bg-transparent border-none cursor-pointer transition-all duration-300 ${
                      index !== activeRouteIndex ? "opacity-40 hover:opacity-70" : "opacity-100"
                    }`}
                  >
                    ...
                    <div className="w-5 h-5 relative">
                      <Image
                        src={index === activeRouteIndex ? "/images/trek-dot-active.svg" : "/images/trek-dot.svg"}
                        alt=""
                        width={20}
                        height={20}
                    ...
                    <p className="font-baron text-base md:text-lg text-[#d4af37] text-center whitespace-nowrap">
                       {route.title.length > 20 ? route.title.slice(0, 18) + '...' : route.title}
                    </p>
                  </button>
                ))}
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between pointer-events-none">
          ...
          <div className="flex gap-3 items-center pointer-events-auto">
            <button
              ...
              onClick={handlePrevGroup}
            >
              <Image src="/images/nav-prev.svg" alt="Previous Start Point" width={44} height={44} className="rotate-180" />
            </button>
            <button
              className="w-11 h-11 hover:opacity-80 transition-opacity"
              onClick={handleNextGroup}
            >
              <Image src="/images/nav-next.svg" alt="Next Start Point" width={44} height={44} />
            </button>
          </div>
        </div>
      </div>
