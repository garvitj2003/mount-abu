...
export default function CustomDropdown({
  ...
  return (
    <div className={`relative h-[38px] ${className}`} style={{ width }} ref={containerRef}>
      <button
        type="button"
      ...
      >
        <div className="flex h-full flex-col justify-center items-start overflow-hidden text-left flex-1">
          ...
          {!isSelected && placeholder && (
            <span className="truncate w-full text-[10px] opacity-40 leading-none">
              {placeholder}
            </span>
          )}
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute left-0 right-0 top-[calc(100%+4px)] z-[100] flex max-h-[240px] flex-col overflow-y-auto rounded-lg border border-[#D6D9DE] bg-white p-1 shadow-[0px_6px_12px_0px_rgba(0,0,0,0.15)] focus:outline-none 
