...
interface NewCategoryDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  data?: ComplaintCategoryResponse | null;
}
...
export default function NewCategoryDrawer({
  ...
  const { mutate: createCategory, isPending: isCreating } = useCreateComplaintCategory();
  const { mutate: updateCategory, isPending: isUpdating } = useUpdateComplaintCategory();
  ...
  const handleSubmit = () => {
    ...
    if (isEdit && data) {
      updateCategory({ id: data.id, data: payload }, {
        onSuccess: () => {
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 300 }} className="relative z-10 h-full w-full max-w-[480px] bg-white shadow-2xl font-onest flex flex-col">
            <div className="flex items-center justify-between border-b border-[#D6D9DE] bg-[#F5F6F7] px-6 py-4">
              <h2 className="text-[15px] font-medium text-[#343434]">{isEdit ? "Edit Category" : "Add New Category"}</h2>
