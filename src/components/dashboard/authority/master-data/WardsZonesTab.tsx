...
type Ward = components["schemas"]["WardResponse"];
...
interface TableColumn {
  ...
  key: string;
  width?: string;
  render?: (row: Ward) => React.ReactNode;
}
...
export default function WardsZonesTab({
  page,
  limit,
  setPage,
}: WardsZonesTabProps) {
  ...
  const totalPages = Math.ceil(filteredWards.length / limit);
  const paginatedWards = filteredWards.slice((page - 1) * limit, page * limit);
          ...
          <table className="w-full min-w-[900px] border-collapse">
            <thead>
              <tr className="border-b border-[#D6D9DE]">
                {WARD_COLUMNS.map((col, idx) => (
                  <th key={idx} className="px-2 py-3 text-left">
                    <div
                      className={`flex items-center gap-2 ${idx < WARD_COLUMNS.length - 1 ? "border-r border-[rgba(0,0,0,0.1)]" : ""} pr-2`}
                    >
                      ...
                      <td key={colIdx} className="px-2 py-3">
                        {col.render ? (
                          ...
                          >
                            ...
                            {String(row[col.key as keyof Ward] || "—")}
                          </span>
                        )}
                      </td>
                    ))}
