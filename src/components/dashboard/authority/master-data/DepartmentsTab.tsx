...
interface TableColumn {
  ...
  key: string;
  width?: string;
  render?: (row: Department) => React.ReactNode;
}
...
const formatCreator = (row: Department) => {
...
export default function DepartmentsTab({
  ...
  const filteredDepartments = departments.filter(
    (dept) =>
      ...
      dept.type.toLowerCase().includes(search.toLowerCase()),
  );
          ...
          <table className="w-full min-w-[900px] border-collapse">
            <thead>
              <tr className="border-b border-[#D6D9DE]">
                {DEPARTMENT_COLUMNS.map((col, idx) => (
                  <th key={idx} className="px-2 py-3 text-left">
                    <div
                      className={`flex items-center gap-2 ${idx < DEPARTMENT_COLUMNS.length - 1 ? "border-r border-[rgba(0,0,0,0.1)]" : ""} pr-2`}
                    >
            ...
            <tbody>
              {paginatedDepartments.length > 0 ? (
                paginatedDepartments.map((row: Department, idx: number) => (
                  <tr
                    key={idx}
                    className="border-b border-[#D6D9DE] hover:bg-gray-50 transition-colors"
                  >
                    {DEPARTMENT_COLUMNS.map((col, colIdx) => (
                      <td key={colIdx} className="px-2 py-3">
                        {col.render ? (
                          ...
                          >
                            {String(row[col.key as keyof Department] || "—")}
                          </span>
                        )}
                      </td>
                    ))}
